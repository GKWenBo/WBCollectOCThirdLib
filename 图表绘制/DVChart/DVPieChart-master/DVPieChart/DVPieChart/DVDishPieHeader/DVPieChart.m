...
-(void)drawRect:(CGRect)rect{
    ...
    CGPoint center =  CGPointMake(self.bounds.size.width * 0.5, self.bounds.size.height * 0.4);
...
//        [self drawLineWithPointArray:pointArray centerArray:centerArray];
    ...
    frame.origin = CGPointMake(self.frame.size.width * 0.5 - frame.size.width * 0.5, self.frame.size.height * 0.4 - frame.size.width * 0.5);
...
//    centerView.nameLabel.text = self.title;
    ...
    UIView *bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, 250, self.frame.size.width, 100)];
    ...
    CGFloat x = 0;
    CGFloat y = 0;
    CGFloat w = (self.frame.size.width - 4*margin)/3.0;
    CGFloat h = 30;
    NSInteger row = 0;
    NSInteger col = 0;
    for (NSInteger i = 0; i<self.colorArray.count; i++) {
        row = i%3;
        col = i/3;
        x = margin + (margin +w)*row;
        y = margin + (margin + h)*col;
        XMGBottonView *view = [[XMGBottonView alloc] init];
        [bottomView addSubview:view];
        view.iconColor = self.colorArray[i];
